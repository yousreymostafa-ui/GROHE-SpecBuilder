(() => {
  ...
  async function open(name,version,stores){
    return new Promise((resolve,reject)=>{
      ...
      req.onupgradeneeded=()=>{const db=req.result;for(const store of stores){if(!db.objectStoreNames.contains(store.name))db.createObjectStore(store.name,store.options||{});}};
