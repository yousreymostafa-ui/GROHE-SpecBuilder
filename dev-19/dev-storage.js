...
async function open(name,version,stores){const devName=String(name||'GROHE').includes('dev19')?String(name):String(name||'GROHE')+'-dev19';return new Promise((resolve,reject)=>{const req=indexedDB.open(devName,version);req.onupgradeneeded=()=>{const db=req.result;for(const store of stores){if(!db.objectStoreNames.contains(store.name))db.createObjectStore(store.name,store.options||{});}};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);});}
...
window.GROHEStorage=Object.freeze({open,getAll,get,put,remove,clear,devLocalOnly:true});
})();